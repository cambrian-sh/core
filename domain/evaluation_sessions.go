...
// set of per-step BudgetLease IDs that are currently part of a sandboxed evaluation
...
func NewInMemoryEvaluationSessions() *InMemoryEvaluationSessions {
	return &InMemoryEvaluationSessions{tokens: make(map[LeaseID]struct{})}
}

// Mark records a session token as an active evaluation session. A no-op on empty.
func (s *InMemoryEvaluationSessions) Mark(sessionTokenID LeaseID) {
	if sessionTokenID == "" {
		return
	}
	s.mu.Lock()
...
func (s *InMemoryEvaluationSessions) Unmark(sessionTokenID LeaseID) {
	if sessionTokenID == "" {
		return
	}
	s.mu.Lock()
...
func (s *InMemoryEvaluationSessions) IsEvaluation(sessionTokenID LeaseID) bool {
	if sessionTokenID == "" {
		return false
	}
	s.mu.RLock()

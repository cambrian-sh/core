...
| `watch-deadletter` contract skew (ADR-0061) | Kernel serves contract `0051` (+ cap `watch-deadletter`, RPC `ListWatchDeadLetters`); `ui/` and `cli/` vendored protos are not re-vendored yet (they already trailed at `0047`). Recorded skew — the handshake degrades clients gracefully; re-vendor is follow-up. |
...
- **ReactiveEngine**, the premium rule engine (`cambrian-premium/reactive/`) that fans a signal out to matching watches, evaluates each condition, and executes the action; it implements `domain.SignalReceiver`, wired via the `app.Options.NewSignalReceiver` seam.

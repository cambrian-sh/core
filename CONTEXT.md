...
| Operator contract skew (ADR-0061 → 0072) | Kernel now serves contract `0055` (accreted caps `watch-deadletter`/`reactive-backpressure`/`watch-condition-guard`/`watch-observability`/`watch-schedule` over REACT-01→06); `ui/` and `cli/` vendored protos are not re-vendored yet (they already trailed at `0047`). Recorded skew — the handshake degrades clients gracefully; batch re-vendor is a follow-up. |
